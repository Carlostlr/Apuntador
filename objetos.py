...
#con esto creamos los botones cada uno con su posicion y ademas con la llamada a su funcion para cambiar de frame
    def boton(self):
        ...
        self.nombre.grid(row=self.row,column=self.column,sticky="e",pady=30,padx=15)
...
#con esto creamos  el label con el nombre del autor abajo a la izquierda
    ...
    def atras(self):
        self.nombre=Button(self.frame, text=self.nombre,width=7, height=2,bg="#545856",fg="white",command=self.valor)
        self.nombre.grid(row=self.row,column=self.column,sticky="e",pady=10,padx=20)
        self.nombre.config(cursor="hand2")

    def peque(self):
        self.nombre=Button(self.frame, text=self.nombre,width=5, height=1,bg="#545856",fg="white",command=self.valor)
...
#los valores se pasan desde la creacion del objeto
    ...
    def update(self):
        ...
        micursor.execute("UPDATE "+ self.tabla + " SET NOMBRE=?, EXPLICACION=?, EJEMPLO=? WHERE NOMBRE=?",(self.valor1,self.valor2,self.valor3,self.valor1))
    ...
    def delete(self):
        ...
        micursor.execute("DELETE FROM "+self.tabla+" WHERE NOMBRE=?",(datos))
